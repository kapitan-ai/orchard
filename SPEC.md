...
The predicate's universe SHALL be every loaded placement of the requested model on an active trusted Node, including placements that ordinary §5.5 eligibility currently excludes, ordered by the §5.7 ranking as it would apply to those placements with the deterministic lexicographic `node_id` tie-break last.
...
When a proving placement exists but cannot be dispatched because aggregate slots or Dispatch Headroom are exhausted, placement concurrency is exceeded, tenant active capacity is exhausted, or a circuit breaker suppresses dispatch, and likewise when the wave deadline elapses with loaded placements still unobserved, the Request SHALL keep its existing `cluster_busy` or `model_busy` outcome and stay queue-waitable under §5.4's controller queue contract; that is transient pre-dispatch unavailability, and neither the capacity-blocked nor the unobserved placement SHALL be read as proof of no reasoning support.
Only when the requested model has no loaded placement on an active trusted Node, or when the §7.5.3a bounded wave observes every such placement and none proves the exact tuple, SHALL Orchard fail the Request closed before dispatch through the §7.2.7 `503 server_error` plus `runtime_incompatible` mapping.
...
The predicate SHALL NOT reclassify ordinary capacity scarcity as incompatibility. The §7.2.7 `503 server_error` plus `runtime_incompatible` pre-dispatch mapping SHALL apply only when the requested model has no loaded placement on an active trusted Node, or when the bounded wave observes every such placement and none proves the exact tuple. When a proving placement exists but cannot be dispatched because aggregate slots or Dispatch Headroom are exhausted, placement concurrency is exceeded, tenant active capacity is exhausted, or a circuit breaker suppresses dispatch, and likewise when the wave deadline elapses with loaded placements still unobserved, the Request SHALL take its existing `cluster_busy` or `model_busy` outcome and remain queue-waitable under §5.4's controller queue contract. Both of those are transient pre-dispatch unavailability rather than proven incompatibility: neither a capacity-blocked nor an unobserved loaded placement SHALL be read as proof of no support, and neither outcome SHALL record `retry_decision = not_retryable`. That equivalence is limited to the queue outcome semantics: §12.4 still resolves the negotiated deadline through the loaded-only formula, so queue wait and wave time consume the same remaining request budget rather than a widened one.

The live observation SHALL run as one bounded wave over the full loaded-placement universe rather than the capacity-eligible subset: every loaded placement of the requested model on an active trusted Node, deduplicated on the same normalized target identity §5.5 uses for the candidate universe, ordered by the §5.7 ranking as it would apply to those placements with the deterministic lexicographic `node_id` tie-break last. Observing the capacity-blocked placements too is what lets exhaustion prove absent support rather than merely absent free capacity.
