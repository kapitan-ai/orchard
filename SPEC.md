...
When the requested model has no loaded placement on an active trusted Node, or when the §7.5.3a bounded wave exhausts the ranked Tier 0 list without a candidate proving the exact tuple, Orchard SHALL fail the Request closed before dispatch through the §7.2.7 `503 server_error` plus `runtime_incompatible` mapping.
When a loaded placement exists but ordinary capacity scarcity leaves no Tier 0 candidate, and likewise when the wave deadline elapses with ranked candidates still unobserved, the Request SHALL keep its existing `cluster_busy` or `model_busy` outcome and stay queue-waitable under §5.4's controller queue contract; neither the capacity-blocked nor the unobserved placement SHALL be read as proof of no reasoning support.
...
The predicate SHALL NOT reclassify ordinary capacity scarcity as incompatibility. The §7.2.7 `503 server_error` plus `runtime_incompatible` pre-dispatch mapping SHALL apply only when the requested model has no loaded placement on an active trusted Node, or when the bounded wave exhausts the ranked Tier 0 list without a candidate proving the exact tuple. When a loaded placement exists but no Tier 0 candidate remains because aggregate slots or Dispatch Headroom are exhausted, placement concurrency is exceeded, tenant active capacity is exhausted, or a circuit breaker suppresses dispatch, and likewise when the wave deadline elapses with ranked candidates still unobserved, the Request SHALL take its existing `cluster_busy` or `model_busy` outcome and remain queue-waitable under §5.4's controller queue contract. Neither a capacity-blocked nor an unobserved loaded placement SHALL be read as proof of no support, and neither transient outcome SHALL record `retry_decision = not_retryable`. That equivalence is limited to the queue outcome semantics: §12.4 still resolves the negotiated deadline through the loaded-only formula, so queue wait and wave time consume the same remaining request budget rather than a widened one.

The live observation SHALL run as one bounded wave per Inference Attempt. Its universe SHALL be built without the reasoning predicate: ordinary §5.5 eligibility, §5.6 Tier 0 grouping, then the §5.7 ranking in force with its deterministic lexicographic `node_id` tie-break last, then deduplication on the same normalized target identity §5.5 uses for the candidate universe. The wave SHALL hold at most four probes in flight and SHALL advance through that deterministic order as probes complete, observing each candidate at most once. Because the §5.7 ranking is capability-blind, a fixed leading window would strand reasoning-capable capacity behind incapable higher-ranked candidates; the advancing window SHALL NOT do so. One **2000 ms** deadline SHALL bound the whole wave rather than each target, and the wave SHALL stop at the first candidate that proves the exact tuple, on exhaustion of the ranked list, or when that deadline elapses, whichever comes first. An individual probe transport SHALL NOT be retried. Exhausting the ranked list proves that no loaded candidate supports the tuple; an elapsed deadline proves only that the unobserved remainder was never reached.

Automatic Attempt Retry SHALL run its own fresh bounded wave rather than reusing attempt 1's evidence, because forwarding SHALL NOT refresh a freshness budget and stale evidence proves no support. Attempt 2's wave SHALL apply `exclude_node_ids` first, rebuild and reorder its universe by that same deterministic rule, advance through it under the same four-in-flight bound and one **2000 ms** wave deadline with no transport retry, and obtain a new `PrepareInference` proof for the different selected endpoint. Each Inference Attempt SHALL run at most one wave, so a logical Request SHALL run at most two, and both waves SHALL draw on the same absolute deadline §12.4 assigned. When that wave proves no different eligible candidate, the retry decision SHALL resolve to `no_alternative_node` unless an earlier reason in the §5.8 decline precedence applies.
...
Queue wait and each attempt's §7.5.3a reasoning wave SHALL consume that same budget rather than extend it, so a negotiated Request matches legacy traffic in queue outcome semantics but not in deadline duration.
