defmodule OrchardCLI.Commands.TLS do
      ...
      server_exists? and not ca_exists? and not force? ->
        {:error,
         "Error: existing server certificate files found without an Orchard CA.\n" <>
           "These may be externally managed. Use --force to overwrite.",
         1}

      ca_exists? and server_exists? and not force? ->
        {:error,
         "Error: certificates already exist. Use --force to regenerate.",
         1}
    ...
    if DateTime.compare(intended_expiry, ca_expiry) == :gt do
      ...
      {:error,
       "Error: server cert validity (#{server_days} days) would exceed CA expiry " <>
         ...
         "Use shorter --server-days or --force to regenerate the CA.",
       1}
    else
      :ok
    end
  ...
  defp check_days_against_ca(false = _reuse_ca?, _ca_crt_path, ca_days, server_days, _runtime) do
    if server_days > ca_days do
      {:error,
       "Error: --server-days (#{server_days}) exceeds --ca-days (#{ca_days}).",
       1}
    else
      :ok
    end
    ...
    case File.read(meta_path) do
      {:ok, contents} ->
        case Jason.decode(contents) do
          {:ok, %{"source" => "generated_local_ca"}} -> :ok
          {:ok, _} -> {:error, "Error: CA was not generated by Orchard (unsupported source type).", 1}
          {:error, _} -> {:error, "Error: metadata file is corrupt: #{meta_path}", 1}
        end
  ...
  defp check_can_trust(runtime) do
    ...
    cond do
      ...
      runtime.uid.() != 0 ->
        {:error, "Error: root privileges required to trust CA.\nRun: sudo orchardctl tls trust-ca", 1}
  ...
  defp generate_ca(staging_dir, ca_days, runtime) do
    ...
    case runtime.cmd.("openssl", ["genrsa", "-out", ca_key, to_string(@ca_key_bits)], cd: staging_dir) do
      {:ok, _} -> :ok
      {:error, _status, output} -> throw({:error, "Error: failed to generate CA private key.\n#{String.trim(output)}", 1})
    end

    # Generate self-signed CA cert
    case runtime.cmd.("openssl", [
           "req", "-new", "-x509",
           "-key", ca_key,
           "-out", ca_crt,
           "-days", to_string(ca_days),
           "-config", ca_cnf
         ], cd: staging_dir) do
      {:ok, _} -> :ok
      {:error, _status, output} -> throw({:error, "Error: failed to generate CA certificate.\n#{String.trim(output)}", 1})
    end
  end
  ...
  defp generate_server_cert(staging_dir, common_name, san_dns, san_ip, server_days, runtime) do
    ...
    case runtime.cmd.("openssl", ["genrsa", "-out", server_key, to_string(@server_key_bits)], cd: staging_dir) do
      {:ok, _} -> :ok
      {:error, _status, output} -> throw({:error, "Error: failed to generate server private key.\n#{String.trim(output)}", 1})
    end
    ...
    case runtime.cmd.("openssl", [
      ...
      {:ok, _} -> :ok
      {:error, _status, output} -> throw({:error, "Error: failed to generate certificate signing request.\n#{String.trim(output)}", 1})
    end
    ...
    case runtime.cmd.("openssl", [
           ...
           "-CAkey", ca_key,
           "-out", server_crt,
           "-days", to_string(server_days),
           "-extfile", ext_cnf,
           "-set_serial", "0x#{serial_hex}"
         ], cd: staging_dir) do
      {:ok, _} -> :ok
      {:error, _status, output} -> throw({:error, "Error: failed to sign server certificate.\n#{String.trim(output)}", 1})
    end
  end
  ...
  defp validate_common_name(cn) do
    ...
    cond do
      ...
      String.contains?(trimmed, [" ", "\t", ",", "\n", "\r"]) ->
        {:error,
         "Error: --common-name must not contain whitespace or commas: #{inspect(cn)}",
         1}

      String.contains?(trimmed, "/") ->
        {:error,
         "Error: --common-name must not contain '/' (X.501 separator): #{inspect(cn)}",
         1}

      String.contains?(trimmed, "\0") ->
        {:error,
         "Error: --common-name must not contain null bytes: #{inspect(cn)}",
         1}

      ip_literal?(trimmed) ->
        {:error,
         "Error: --common-name must not be an IP address (use --ip instead): #{cn}",
         1}
  ...
  defp validate_positive(name, value) do
    {:error, "Error: --#{name |> to_string() |> String.replace("_", "-")} must be a positive integer, got: #{value}", 1}
  end
