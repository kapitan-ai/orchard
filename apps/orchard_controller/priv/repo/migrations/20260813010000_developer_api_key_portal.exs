defmodule Orchard.Repo.Migrations.DeveloperApiKeyPortal do
  ...
  def change do
    ...
    create(
      constraint(:portal_users, :portal_users_status_closed,
        check: "status IN ('invited', 'active', 'disabled')"
      )
    )

    create(
      constraint(:portal_users, :portal_users_session_epoch_non_negative,
    ...
    create table(:portal_invite_tokens, primary_key: false) do
      ...
      add(
        :portal_user_id,
        references(:portal_users, type: :binary_id, on_delete: :delete_all),
        null: false
      )
    ...
    create(
      constraint(:portal_invite_tokens, :portal_invite_tokens_hash_length,
    ...
    create(
      constraint(:api_keys, :api_keys_developer_portal_tenant_direct,
        check:
          "issuance_surface <> 'developer_portal' OR " <>
            "(tenant_id IS NOT NULL AND service_account_id IS NULL)"
      )
    )

    create(
      index(:api_keys, [:portal_user_id, :inserted_at],
        name: :idx_api_keys_portal_user_active,
        where:
          "portal_user_id IS NOT NULL AND issuance_surface = 'developer_portal' " <>
    ...
    create(
      constraint(:portal_login_throttles, :portal_login_throttles_identity_fingerprint_length,
        check: "octet_length(identity_fingerprint) = 32"
      )
    )
