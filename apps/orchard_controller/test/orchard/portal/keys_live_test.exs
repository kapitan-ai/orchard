defmodule Orchard.Portal.KeysLiveTest do
  ...
  setup do
    ...
    {:ok, session} = Governance.create_portal_session("portal-keys", @password, "203.0.113.50")

    %{tenant: tenant, token: session.token}
  end
  ...
  test "password rotate fails revoke on a standing keys socket", %{
    conn: conn,
    token: token,
    tenant: tenant
  } do
    ...
    {:ok, view, _html} = live(authed(conn, token), "/portal/portal-keys/keys")
    assert {:ok, _} = Governance.set_tenant_portal_password(tenant, "sixteen-chars-two")
