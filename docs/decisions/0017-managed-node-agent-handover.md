...
6. **Matching is observable without the lock, and a claimed child stays provisional.** Because the gate is barred from the lock, the crash fence cannot rest on owner liveness inferred from it. The authorization therefore carries components the gate can check directly — attempt identity, exact non-reusable owner process identity, per-bootstrap nonce, launchd label, expected generation and executable identity, eligibility generation, claim state — and requires the recorded owner instance to be observably live. A claimed child records its own exact identity and runs provisional: no cluster identity, no serving, watching that owner instance, until the same owner atomically records terminal coherent evidence and `enabled` eligibility bound to it. This is what makes a mid-attempt owner death safe with no live actor to re-apply disablement; owner death after acceptance is ordinary and leaves the instance valid.
...
`SPEC.md` sections 11.2, 11.4, and 13.4 define the orthogonal lifecycle state model, reboot-safe Node Agent start suppression through persistent launchd job-domain disablement beneath `RunAtLoad` and `KeepAlive`, the owner-side and child-side split of the exclusion boundary, the lock-held managed stop protocol, per-entry-state start dispatch, the observable one-shot matching predicate and provisional child phase, authenticated single-generation PKG activation, the single-owner crash-released lock, pre-`bootout` process capture, required durable recovery evidence, verify-or-establish start preconditions, manual PKG start, unconditional Orchard.app rollback, managed recovery, and historical-version serialization.
