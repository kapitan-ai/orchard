...
Dispatch bounds its own acquisition of that gate by the request timeout and fails with the same reason code, so a Node that never accepts cannot make every other dispatch to it wait indefinitely.
When a dispatch cannot resolve whether its runtime execution ended — a cancel drain that times out without a clean disconnect and without a durably recorded unreachable or unhealthy Node — the authority quarantines that Node, and every later evaluation for it is treated as unreachable rather than trusted as free capacity.
