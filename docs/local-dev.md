...
| `ORCHARD_CONTROLLER_MEMBERSHIP_HOST` | the `ORCHARD_BEAM_NODE_NAME` host, or `127.0.0.1` under `grpc` | Controller-only override for the private IPv4 address that names the Controller's durable membership identity. It must be a private IPv4 literal and must match the `ORCHARD_BEAM_NODE_NAME` host under `beam`. Only controller and all-in-one roles resolve it; a node-agent-only host ignores it. |
...
`orchardctl nodes inspect` renders a counterfactual dispatch-capacity block in both human and `--json` output, showing what F11 enforcement would decide without changing dispatch behavior; under `pre_cutover` it reports Effective Dispatch Limit and Dispatch Headroom `0`, exposes temporary legacy slots separately, and reports `consumers_ready` as `false` for this non-enforcing slice.
