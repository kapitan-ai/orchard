...
Managed Postgres is not available in current builds. Controller-bearing packaged
installs require an external PostgreSQL server; the current PKG ships only an
operator-safe `orchard-managed-postgres` guard and excludes the Postgres
LaunchDaemon. See `../pkg/README.md` and `postgres/README.md`.
