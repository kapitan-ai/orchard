...
def finalize(ctx: ToolCallingContext) -> BackendError | None:
    if ctx.text_buffer:
        ctx.pending_events.append({"kind": "output_text_delta", "delta": ctx.text_buffer})
        ctx.text_buffer = ""

    if ctx.in_tool_call:
        ctx.current_tool_text += ctx.tool_end_buffer
        ctx.tool_end_buffer = ""
        if ctx.current_tool_text:
            _append_tool_events(ctx, ctx.current_tool_text)
    ...
    if ctx.requires_tool_call and not ctx.saw_tool_call:
...
def _consume_tool_text(ctx: ToolCallingContext, text: str) -> str:
    ...
    if end_pos == -1:
        ...
        ctx.current_tool_text += safe_text
        ctx.tool_end_buffer = suffix
        return ""

    ctx.current_tool_text += combined[:end_pos]
    if ctx.current_tool_text:
        _append_tool_events(ctx, ctx.current_tool_text)
        ctx.current_tool_text = ""
    ctx.in_tool_call = False
...
def _append_tool_events(ctx: ToolCallingContext, tool_text: str) -> None:
    if tool_text == "":
        return

    try:
        parsed = ctx.tool_parser(tool_text, ctx.tools)
    ...
    for parsed_call in parsed_calls:
        normalized = _normalize_parsed_tool_call(parsed_call)
        ...
        tool_call_id = normalized["id"] or f"call_{ctx.next_tool_index}"
...
def _normalize_parsed_tool_call(parsed_call: Any) -> dict[str, str | None]:
    ...
    return {"id": tool_call_id, "name": name, "arguments": arguments_json}
