...
- [ ] 3.1 Complete and merge issue #326 canonical normalization and exact rendering without changing omitted-request behavior; add `reasoning` to both public validators only at activation, narrow the shared `validate_reasoning_combination!/5` so `explicit_public + model_default` is rejected, and add shared canonical-request regression coverage for that combination.
