...
Rebuilt exception, thread, thread-stacktrace, request, and stack-frame containers SHALL be the interface structs the pinned renderer requires, never bare maps. Orchard MAY still reduce a map-shaped stacktrace supplied by non-SDK compatibility input to a frames-only map, because the pinned renderer omits that unsupported non-struct interface before serialization rather than emitting it.
