...
- Preserve only validated first-party repo-relative stack-frame filenames under `apps/<first-party-app>/lib/`; remove absolute, machine-specific, traversal, unknown, and source URL paths, including path-shaped values placed in frame or exception names.
