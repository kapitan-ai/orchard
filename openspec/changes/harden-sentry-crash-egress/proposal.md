...
- Derive an `orchard_thread_stack_hash` extra from the already-retained thread frame fields so different thread-only crash sites stay distinguishable to the pinned SDK deduplication hash, without changing fingerprints, grouping, or deduplication settings.
