...
- Preserve the normative `PrepareInference` behavioral barrier: its proof and opaque single-use authorization form the pre-inference gate, and only the matching execution request redeems the authorization after Controller proof validation. The owner-confirmed documentation record fixes the WorkerRuntimeService-only protobuf RPC declaration owner, its transport-independent Controller-facing Runtime Endpoint Interface counterpart, exact preparation/proof/redemption layout, 32-byte authorization, monotonic TTL, full binding, and atomic single-use semantics; this PR adds no source declaration or runtime behavior.
- Trace, rather than re-choose, the `SPEC.md` §7.5.3a allocation: `WorkerCapabilities` field 8 for `WorkerLoadedBinding`, field 9 for its sibling reasoning envelope, and shared tuple, evidence, preparation, and proof definitions in `proto/cluster/v1/reasoning.proto`. The owner-confirmed record assigns the complete tags, types, presence, and identity rules in `design.md` §2.1. Implementation re-confirms those allocations and blocks rather than substituting a conflicting number or shape.
- Record the complete owner-confirmed schema design — enum values, `nil`-presence encoding, service and RPC declaration ownership, message layouts, terminal usage, typed-event deferral, and execution-redemption shape — in `design.md` §2.1. This PR adds no protocol source declaration or generated binding; a separate accepted schema OpenSpec package and durable decision record remain prerequisites for implementation under `design.md` §1 condition 3.
...
- `proto/cluster/v1/reasoning.proto` remains the `SPEC.md` §7.5.3a shared definition location and one `cluster.v1` dependency of the provider-neutral Worker Runtime boundary; its relocation or removal is sequenced by the later `cluster.v1` deprecation rather than by this contract. The owner-confirmed record places all cross-boundary definitions there without adding or altering them in this PR.
- Future implementation may modify shared protocol source, Runtime Endpoint bindings, Worker Runtime bindings, Node Agent, Controller, and provider-neutral fixtures only after PR #401 has merged, this contract is accepted, and the required separate accepted schema package plus durable decision record incorporate the owner-confirmed design without changing it.
