...
- Address each singular live observation with an exact `ModelRef` model/version selector and preserve its selected-worker association through both adapters. Wrong-worker or unattributable results remain unknown, not confirmed non-support.
- Preserve optional/default frozen execution values and bind every future execution-affecting additive or nested input, or reject it before preparation or invocation; validate redemption independently outside the frozen input.
