...
- Reserve the pre-dispatch incompatibility mapping for a model with no loaded placement or a ranked candidate list the wave exhausted without a proving candidate. A placement that is merely at capacity, or one the wave never reached, keeps the ordinary `cluster_busy`/`model_busy` queue-waitable outcome under §5.4 and stays retryable.
- Bound the live wave on concurrency rather than reach: at most four probes in flight advancing down a deterministic order — universe built without the reasoning predicate, then §5.7 ranking with the `node_id` tie-break last — under one 2000 ms wave deadline with no transport retry. Because that ranking is capability-blind, a fixed leading window could strand capable capacity permanently. The explicit negotiated request is the only opt-in; there is no separate operator configuration flag.
