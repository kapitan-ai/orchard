...
- Observe the reachable loaded-placement universe — capacity- and tenant-cap-blocked placements included, stale, unhealthy, and breaker-suppressed Nodes withheld — so exhaustion can distinguish absent support from absent free capacity without circumventing §5.10 suppression.
- Keep selection evidence and exhaustion evidence distinct: an incomplete probe proves no support for selection but leaves that placement's support unknown. Reserve the pre-dispatch incompatibility mapping for a model with no loaded placement, or a universe in which every placement was probed and each affirmatively answered that it does not support the tuple. Capacity-blocked, unreachable, and withheld placements all keep the `cluster_busy`/`model_busy` queue-waitable outcome under §5.4 as transient unavailability.
