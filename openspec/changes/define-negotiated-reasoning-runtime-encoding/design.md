...
Restricting selection to loaded placements must not turn a busy cluster into a permanently incompatible one. Because §5.6 tiers are drawn from the §5.5 eligible set, a momentarily concurrency-blocked placement leaves zero Tier 0 candidates for reasons that have nothing to do with reasoning. The predicate therefore runs after eligibility and ranking and never authors the scarcity outcome: `runtime_incompatible` covers only a model with no loaded placement on an active trusted node, or an observed Tier 0 candidate set in which none proves the tuple. A loaded placement that exists but has no available capacity keeps the ordinary `cluster_busy`/`model_busy` queue-waitable path, and its unobserved reasoning support is not read as proof of absence.

The live wave is bounded like the §5.5 compatibility status-probe wave rather than left to per-candidate fan-out: at most four candidates, one observation attempt each, a 2000 ms per-target timeout, and no transport retry.

"First four" needs an order the §5.5 precedent gets for free from configuration order. Here the universe is built without the reasoning predicate — ordinary §5.5 eligibility, §5.6 Tier 0 grouping, the §5.7 ranking in force with its lexicographic `node_id` tie-break last — and then deduplicated on the §5.5 normalized target identity. The wave observes the first four of that order, so identical scheduling attempts probe the same candidates and negotiated availability is reproducible. Candidates past the bound stay unobserved and unobserved evidence proves no support; the wave is not extended to reach them.

The budget is per attempt rather than per logical request, because reusing attempt 1's evidence would contradict the freshness rules this contract depends on: forwarding does not refresh a remaining freshness budget, and stale evidence proves no support. Attempt 1 can consume most of the deadline before a pre-commit failure, so attempt 2 runs a fresh wave — `exclude_node_ids` applied first, universe rebuilt and reordered by the same deterministic rule, first four deduplicated candidates, same 2000 ms per target, no transport retry — and obtains a new `PrepareInference` proof for the different endpoint it selects. One wave per attempt caps a logical request at two. A wave that proves no different eligible candidate declines with `no_alternative_node` under the §5.8 precedence.
