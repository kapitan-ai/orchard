...
- Require distinct durable evidence before protected mutation and terminal coherent marking only after the applicable verification without treating metadata as exclusion ownership, using one shared schema whose operation kind is `handover`, `managed_recovery`, `start_attempt`, or `managed_stop`.
- Scope evidence denial to the kind an attempted operation actually requires, exempt managed stop from needing prior coherent evidence, and make an interrupted non-terminal record a reconcile-and-supersede obligation for the next lock-holding owner rather than a full payload-recovery trigger.
...
- Require a claimed child to run provisional, cluster-identity-free and non-serving, and make the owner's single atomic commit of terminal coherent evidence, `enabled` eligibility, and acceptance bound to that exact child the sole serving linearization point, resolved by a fresh authoritative read on observed owner loss rather than by timing.
...
- Define one shared Managed Node Agent Process Fence that handover, managed stop, start-attempt precondition establishment, and managed recovery all satisfy, so observation and capture always precede shutdown and post-shutdown non-observation never becomes proven absence.
- Make the suppressed-state start precondition total over load state and process state, so an unloaded job with a live, additional, replacement, unstable, or unknown managed process never bootstraps a second Node Agent onto the live Node Identity Root.
