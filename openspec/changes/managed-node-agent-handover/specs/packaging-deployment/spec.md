...
`orchardctl start` SHALL acquire the canonical lock and dispatch on the observed eligibility state, running a managed start attempt only from `suppressed`, succeeding idempotently under `enabled` with exactly one verified healthy managed instance, and otherwise normalizing to `suppressed` through managed recovery under the same lock before a distinct attempt runs.
`orchardctl stop` SHALL be an owner-side path that acquires the same lock and satisfies the Managed Node Agent Process Fence, setting eligibility `suppressed`, invalidating outstanding authorization, and applying persistent job-domain disablement before observing process state under suppression immediately before `bootout`, so a PKG-installed host that is stopped and later started always re-enters the start attempt from `suppressed`.
A start attempt SHALL verify prior terminal coherent PKG handover or recovery evidence and coherent installed state, and record distinct non-terminal start-attempt evidence.
While retaining the lock with eligibility still `suppressed`, it SHALL prove both that the launchd job is unloaded and that no managed Node Agent process is running before recording any authorization or bootstrapping: a loaded job requires the Managed Node Agent Process Fence, capturing the exact stable instance or recording affirmative absence under suppression immediately before `bootout` and then proving the job unloaded and every captured instance exited; an already unloaded job with affirmatively proven managed-process absence may proceed; and in any other case, including an unloaded job with a live, additional, replacement, identity-unstable, or unknown managed process, it SHALL fail closed with durable suppression retained.
It SHALL then lift persistent job-domain disablement for exactly one explicit bootstrap, durably record one single-consumer one-shot authorization carrying the start-attempt identity, exact non-reusable owner process identity, per-bootstrap nonce, intended launchd label, expected active or staged generation and executable identity, eligibility generation, and claim state, then bootstrap and verify the intended Node Agent instance.
The child-side launch gate SHALL match those components and require an observably live recorded owner instance without acquiring the canonical lock, SHALL claim at most once, and SHALL NOT reclaim a claimed authorization.
Only after the owner verifies the exact claimed child SHALL it atomically mark the start attempt terminal coherent and enable durable eligibility bound to that child for normal `RunAtLoad` and `KeepAlive` operation.
Failure or owner death before that transition SHALL invalidate authorization, keep or restore durable suppression including persistent job-domain disablement, and cause any provisional Node Agent to exit without serving.
