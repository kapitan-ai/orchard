...
`orchardctl stop` SHALL be an owner-side path that acquires the same lock, sets eligibility `suppressed`, invalidates outstanding authorization, and applies persistent job-domain disablement before `bootout`, so a PKG-installed host that is stopped and later started always re-enters the start attempt from `suppressed`.
