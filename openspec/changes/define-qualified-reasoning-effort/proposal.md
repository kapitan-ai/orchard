...
- Preserve PR #325's enabled-conformance rule for every tier: a tier that cannot prove valid non-empty reasoning for its exact tuple is unsupported. Provider conformance fixtures and loaded-worker acceptance own runtime advertisement and dispatch, while repository-owned qualification owns only what may be offered or represented as supported, so `SPEC.md` §6.4 independence is preserved.
