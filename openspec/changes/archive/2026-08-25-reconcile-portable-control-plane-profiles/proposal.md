...
- Keep profile definition and topology identity separate from support status, database mode, and milestone acceptance.
...
- `platform-profiles`: Renames the portable Orchard control-plane core boundary and the profile requirement, defines platform, distribution, runtime-provider, and acceptance profiles as distinct qualified concepts, and makes support contingent on acceptance evidence rather than profile definition.
...
- `app-distribution-lifecycle`: Requalifies Purpose prose to the macOS native distribution profile without changing any requirement.
  OpenSpec delta grammar does not model Purpose-only edits, so this proposal, its Impact section, and task 1.7 record that provenance without a no-op requirement delta.
...
- `SPEC.md` distinguishes current all-in-one and source-development split-role behavior from packaged multi-Mac acceptance gaps, future Managed Database Mode, and the Active/Standby target.
