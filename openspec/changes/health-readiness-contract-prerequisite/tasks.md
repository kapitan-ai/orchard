...
- [x] 2.5 Verify that stage one covers Console, CLI, packaging, local-development documentation, tests, and `docs/milestones/m0-foundation.md`; defer transport-gate removal and the disposition of `controller_boot_completed` to stage two.
...
- [ ] 3.4 Confirm the behavior-changing pull request excludes transient investigation, review, prompt-export, and machine-local artifacts.
- [ ] 3.5 State that stage one changes exposure and `SPEC.md` without changing the readiness predicate, and that issue #115 remains open for its external acceptance harness.
