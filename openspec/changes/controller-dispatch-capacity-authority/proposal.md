...
It completes that same reconciliation in the §5.9 dispatch sequence by scoping trusted production identity, Controller allocation, and temporary legacy-claim acquisition, retention, and revalidation to the production-managed decisions, by defining the matching unmanaged path as positive decision-specific available slots revalidated with no Controller allocation or temporary legacy claim, and by stating that `fail_closed` never proceeds to `ExecuteInference`.
