...
It completes that same reconciliation in the §5.9 dispatch sequence by scoping that sequence, trusted admitted production inventory resolution, Controller allocation, temporary legacy-claim acquisition, retention, revalidation, and the per-Node acceptance gate to the production-managed decisions, by recording that a valid explicitly classified unmanaged target stays on its documented unmanaged legacy dispatch contract outside those mechanisms, and by stating that `fail_closed` never proceeds to `ExecuteInference`.
