...
- [ ] 5.10 Cover explicit unmanaged source development and compatibility, invalid or missing management class, admitted-inventory override, admitted production over gRPC compatibility, static targets matching admitted production inventory, classification behavior before and after the conjunctive classification authority gate, proof that `dispatch_capacity_consumers_ready = true` alone is not an enforcement switch, and proof that an admitted production target lacking fresh trusted capacity evidence cannot receive new work through unmanaged classification or compatibility fallback at any readiness stage. Broader production probe-failure direct scheduling fallback cleanup remains pending separately.
