...
- Make bulk provisioning create service-account-owned API Tokens by default and leave tenant-direct API Keys as a manual, bootstrap, and compatibility path.
...
- Emit One-time Secret Output only after successful apply and never persist plaintext token secrets in Postgres, audit logs, provisioning batches, Console state after dismissal, support bundles, or local evidence artifacts.
...
- Affected product contract: `SPEC.md` §2.1, §2.3, §3.4, §5.2, §5.3, §7.4.4, §8.2, §8.3, §8.4, §10.2, §10.3, §10.4, §10.9, and §11.9.
