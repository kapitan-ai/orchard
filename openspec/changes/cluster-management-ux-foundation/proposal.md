...
Orchard's cluster management surface is approaching the point where operators need one coherent answer across Console, CLI, Operator API, Admin API, scheduler explanations, diagnostics, and Active/Standby status.
`SPEC.md` already defines node lifecycle, health, scheduler eligibility, Runtime Endpoint observations, operator APIs, admin APIs, diagnostics, and Active/Standby leadership.
...
- Require CLI and Console parity for node list, node detail, admission review, action eligibility, scheduler explanation, diagnostics, and control-plane read-only status.
...
- Define shared diagnostics expectations without introducing a packaged evidence artifact.
...
- `cluster-management-ux`: Operator-facing cluster UX contract for node admission, lifecycle state presentation, reason-code taxonomy, CLI/Console parity, safe node actions, scheduler explanations, diagnostics, and control-plane status.
...
- Current audit impact: implementation slices need cluster-scoped audit events for node admission, rejection, rejection clearance, decommission, and Active/Standby write-path decisions.
...
- Current diagnostics impact: implementation slices must expose bounded node and request diagnostics consistently across operator surfaces without creating a packaged evidence artifact.
- Security impact: diagnostic surfaces must preserve `SPEC.md` §10.2 secret handling and must never include plaintext secrets, credentials, DSNs, prompt bodies, response bodies, or raw local evidence logs unless an accepted capture-mode contract explicitly permits them.
