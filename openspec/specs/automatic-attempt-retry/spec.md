# automatic-attempt-retry Specification

## Purpose
Define Orchard's bounded Automatic Attempt Retry contract: at most one second Inference Attempt on a different eligible Node before Output Commitment, under one logical Request and its original admission, queue, quota, idempotency, capture, and deadline budgets.
